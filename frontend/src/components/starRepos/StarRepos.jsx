...
const StarRepo = () => {
  ...
  const confirmDelete = async () => {
    try {
      setLoadingButtons((prev) => ({ ...prev, delete: true })); // ✅ BUTTON LOADING
      const userId = localStorage.getItem("userId");
      ...
      setLoadingButtons((prev) => ({ ...prev, delete: false })); // ✅ STOP BUTTON LOADING
    }
  };
  ...
  useEffect(() => {
    const fetchStarredRepos = async () => {
      try {
        ...
        setRepositories(Array.isArray(data?.repositories) ? data?.repositories : []);
        setSearchResults(Array.isArray(data?.repositories) ? data?.repositories : []);
        const ids = data?.repositories?.map((repo) => repo?._id?.toString()) || [];
        setStarredRepoIds(ids);
        toast.success(data?.message || "Starred repositories fetched successfully");
      } catch (error) {
        console.error(error);
  ...
  const handleToggleVisibility = async (repoId) => {
    try {
      setLoadingButtons((prev) => ({ ...prev, [`visibility-${repoId}`]: true })); // ✅ BUTTON LOADING
      setLoading(true);
    ...
    <>
      ...
      <nav className="nav1JKLPPOPOIUT">
        {!isMobile && (
          <div>
            <div>
              <BookOpenIcon className="iconnJKLPPOPOIUT" /> Overview
            </div>
            <div>
              <BookMarkedIcon className="iconnJKLPPOPOIUT" /> Repositories
            </div>
            <div>
              <GridIcon className="iconnJKLPPOPOIUT" /> Projects
            </div>
            <div>
              <BoxIcon className="iconnJKLPPOPOIUT" /> Packages
            </div>
            <div>
              <StarIcon className="iconnJKLPPOPOIUT" /> Stars
            </div>
          </div>
        )}
        {isMobile && (
        ...
        <div className="klJKLPPOPOIUT">
          <div className="divOJKLPPOPOIUT">
            ...
            <div className="gh-info1JKLPPOPOIUT">
              ...
              <div>
                <Clock size={16} /> <span>17:45 (UTC +05:30)</span>
              </div>
            </div>
            <div className="gh-highlights1JKLPPOPOIUT">
              <h4>Highlights</h4>
              ...
              {searchResults?.map((repo) => (
                <div key={repo?._id} className="trending-card1JKLPPOPOIUT">
                  ...
                  <div className="repo-meta1JKLPPOPOIUT">
                    ...
                    <span className="repo-stars1JKLPPOPOIUT">⭐ 282</span>
                    {myId === repo["owner"] && (
                      <>
                        <button
                          className="star-btn1JKLPPOPOIUT KRUNAL_PANDYA"
                          ...
                          disabled={loadingButtons[`update-${repo?._id}`]}
                        >
                          {loadingButtons[`update-${repo?._id}`] ? "Loading..." : "Update"}
                        </button>
                        ...
                        >
                          {loadingButtons.delete ? "Loading..." : "Delete"}
                        </button>
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ marginTop: "15px" }}>
                      <button
                        onClick={confirmDelete}
                        style={{ backgroundColor: "#ef4444", color: "white", padding: "6px 12px", marginRight: "10px" }}
                        disabled={loadingButtons.delete}
                      >
                        {loadingButtons.delete ? "Loading..." : "Confirm"}
                      </button>
                      <button
                        onClick={() => setShowDeleteModal(false)}
